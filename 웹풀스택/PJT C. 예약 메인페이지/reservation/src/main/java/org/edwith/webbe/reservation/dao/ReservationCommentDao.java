...
public class ReservationCommentDao {
    ...
    public List<ReservationComment> getComments(Integer start, Integer count) {
    	...
    	reservationComments = reservationComments.stream().map(reservationComment -> {
    		reservationComment
    		.setReservationCommentImages(
				reservationCommentImageDao
				.selectByReservationCommentId(reservationComment.getId())
			);
    ...
    public List<ReservationComment> getComments(Integer productId, Integer start, Integer count) {
    	...
    	params.put("start", start);
    	params.put("count", count);
    	List<ReservationComment> reservationComments = jdbc.query(SELECT_BY_PRODUCTID_LIMIT_START_COUNT, params, rowMapper);
    	reservationComments = reservationComments.stream().map(reservationComment -> {
    		reservationComment
    		.setReservationCommentImages(
				reservationCommentImageDao
				.selectByReservationCommentId(productId)
			);
			return reservationComment;
    	}).collect(Collectors.toList());
    	return reservationComments;
    ...
    public Integer insertReservationComment(
    		int productId, int reservationInfoId, int score, String email, String comment
    	) {
